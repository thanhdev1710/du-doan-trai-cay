...
MODEL_PATH = 'mobilenetv2_best.pth' # Using your specified model path
DATASET_DIR = 'dataset' # Your specified dataset directory
TRAIN_DIR = os.path.join(DATASET_DIR, 'train')
VAL_DIR = os.path.join(DATASET_DIR, 'val')
TEST_DIR = os.path.join(DATASET_DIR, 'test')
NUM_WORKERS = min(8, os.cpu_count()) # Max 8 workers or CPU count
...
def evaluate_and_report_on_test_set(model, test_loader, criterion, class_names):
    ...
    with torch.no_grad():
        for images, labels in progress_bar_report:
            images = images.to(device) # Labels are not needed for prediction on device
            outputs = model(images)
            _, preds = torch.max(outputs, 1)
            y_true.extend(labels.cpu().numpy()) # Move labels to CPU for sklearn
            y_pred.extend(preds.cpu().numpy())  # Move predictions to CPU for sklearn

    print("\nClassification Report:\n")
    try:
        # Ensure labels are within the range of class_names
        unique_labels_in_data = sorted(list(set(y_true) | set(y_pred)))
        
        # Filter class_names and labels for report based on what's actually present and valid
        ...
        if not report_target_names: # If no valid labels/target_names for report
...
if __name__ == "__main__":
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min',
                                                     ...
                                                     factor=0.5) # factor 0.5 is also common
